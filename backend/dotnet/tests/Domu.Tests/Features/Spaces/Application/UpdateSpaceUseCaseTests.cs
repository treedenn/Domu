...
public sealed class UpdateSpaceUseCaseTests
{
    ...
    public async Task ExecuteAsync_UpdatesExistingSpace()
    {
        ...
        var result = await useCase.ExecuteAsync(
            new UpdateSpaceCommand(new DomuActor(Guid.NewGuid(), DomuActorType.Zitadel), space.HouseholdId, space.Id,
                "Kitchen Pantry", "Updated description"),
    ...
    public async Task ExecuteAsync_WhenSpaceDoesNotExist_Throws()
    {
        ...
        var action = () => useCase.ExecuteAsync(
            new UpdateSpaceCommand(new DomuActor(Guid.NewGuid(), DomuActorType.Zitadel), Guid.NewGuid(), Guid.NewGuid(),
                "Pantry", null),
            CancellationToken.None);
