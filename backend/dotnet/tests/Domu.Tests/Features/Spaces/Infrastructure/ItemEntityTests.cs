...
public sealed class ItemEntityTests
{
    ...
    public void FromDomain_AndToDomain_RoundTripsAggregate()
    {
        ...
        Assert.Equal(item.TotalCount, roundTrippedItem.TotalCount);
    ...
    public void UpdateFromDomain_SynchronizesAddedUpdatedAndRemovedEntries()
    {
        ...
        Assert.Contains(entity.Entries, entry => entry.Id == existingEntry.Id && entry.CurrentAmountPerUnit == 3.5m);
        Assert.Contains(entity.Entries, entry => entry.Id == newEntry.Id && entry.CurrentAmountPerUnit == 2);
    }
}
